...
class MapMatching(object):
	def __init__(self):
		...
		# self.video_writer = cv2.VideoWriter('video.avi', cv2.cv.CV_FOURCC('X', 'V', 'I', 'D'), 20, (self.resize_width, self.resize_height), True)
		...
		for i in range(particle_number):
			self.particles.append(
				Particle(
					...
					self.start_pressure + self.pressure_gain,
					self.start_floor,
					random.uniform(0.9, 1.1),           # initialising scale uniformly. generate a uniform random number distribution in the range [a, b) or [a, b] depending on rounding
		...
		self.IPIN_filename = 'log\\amit.csv'
	...
	def MapToWorld(self, _x, _y, _floor):                   # how to get the values
		homography = [
	...
	def BackTrackingParticle(self, _particle, _step, _position_check_number=0, _position_error_max=2.0):
		...
		for tStep in _step:
			...
			if temp_position_check_number > 0:
				temp_distance = math.sqrt((current_x - tStep.current_position.x) ** 2 + (current_y - tStep.current_position.y) ** 2) #distance of a line formula when 2 points are given
	...
	def ShowMap(self):
		...
		resized_image = cv2.resize(image_show, (960, 540))
		cv2.imshow(self.window_name, resized_image)
		cv2.waitKey(5)
		# self.video_writer.write(resized_image)
		####added####3
		print('current_position x', self.current_position.x)
		print('current_position y', self.current_position.y)
		print('current heading i.e. angle', self.current_position.z)
...
#### main part
	def Update(self, _dx, _dy, _z, _check_floor=True):
		...
		if particle_number > 0:
			self.current_position.x = x_sum / particle_number  # final position P, equation 4
		...
		if particle_number > 0:
			...
			while new_particle_number > 0:
				slice_particles = random.sample(self.particles, sample_number)   # algorithm 1
				x_sum = 0.0
				y_sum = 0.0
				angle_sum = 0.0
