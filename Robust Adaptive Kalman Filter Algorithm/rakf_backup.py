...
for temp_line in in_file.readlines():
    ...
    heading_estimate = 0
    heading_estimate = RAKF1D(initial_state=1,
                 system_model=1,
                 system_model_error=1,
                 measurement_error=96,
                 state_error_variance=7,
                 residual_threshold=0.2, adaptive_threshold=0.1,
                 estimator_parameter_count=10,
                 gamma=50,
                 model_type="uwb_imu")

    heading_estimate.run(yaw,0,0,0)
    estimated_heading=heading_estimate.state_model
    print(estimated_heading)
    # print("heading", heading_estimate.state_model)
    # plt.plot(yaw)
    # plt.scatter(time, yaw, c='b', marker='x', label='1')
    # plt.scatter(time, estimated_heading, c='b', marker='x', label='1')
    # plt.plot(estimated_heading)
